...
class NavigationClient(Node):
    def __init__(self, node_name='navigation_client'):
        ...
        self.get_logger().debug('Cliente de navegación inicializado')
    ...
    def wait_for_action_server(self, timeout_sec=5.0):
        if not self.nav_client_.wait_for_server(timeout_sec=timeout_sec):
            self.get_logger().error('Servidor de navegación no disponible tras espera')
            return False
        self.get_logger().debug('Servidor de navegación disponible')
        return True
    ...
    def send_goal(self, target_pose: PoseStamped):
        ...
        self.get_logger().debug(f'Enviando objetivo: ({target_pose.pose.position.x:.2f}, {target_pose.pose.position.y:.2f})')
    ...
    def goal_response_callback(self, future):
        ...
        self.get_logger().debug('Objetivo aceptado, navegación iniciada')
    ...
    def result_callback(self, future):
        ...
        if status == GoalStatus.STATUS_SUCCEEDED:
            self.get_logger().debug('Navegación completada con ÉXITO')
            self.goal_success_ = True
        elif status == GoalStatus.STATUS_ABORTED:
            self.get_logger().warn('Navegación ABORTADA (obstáculo o timeout)')
            self.goal_success_ = False
        elif status == GoalStatus.STATUS_CANCELED:
            self.get_logger().warn('Navegación CANCELADA')
            self.goal_success_ = False
        else:
            self.get_logger().error(f'Estado desconocido: {status}')
            self.goal_success_ = False

    def cancel_goal(self):
        if self.goal_handle_ and self.goal_active_:
            self.get_logger().debug('Cancelando objetivo de navegación')
    ...
    def wait_for_result(self, timeout_sec=300.0):
        ...
        while rclpy.ok():
            if self.goal_done_:
                self.get_logger().debug(f'Goal finalizado: {"ÉXITO" if self.goal_success_ else "FALLO"}')
                return self.goal_success_
            
            elapsed = time.time() - start_time
            if elapsed > timeout_sec:
                self.get_logger().warn(f'Timeout esperando resultado del goal ({timeout_sec} s)')
                self.cancel_goal()
                return False
